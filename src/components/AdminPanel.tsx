...
interface AdminPanelProps {
  ...
  darkMode: boolean;
  toggleDarkMode: () => void;
  activeTab?: 'dashboard' | 'cafes' | 'sales' | 'products' | 'reports' | 'settings';
  setActiveTab?: (tab: 'dashboard' | 'cafes' | 'sales' | 'products' | 'reports' | 'settings') => void;
}
...
export default function AdminPanel({
  ...
  const [localActiveTab, setLocalActiveTab] = useState<'dashboard' | 'cafes' | 'sales' | 'products' | 'reports' | 'settings'>('dashboard');
  const activeTab = propActiveTab !== undefined ? propActiveTab : localActiveTab;
  const setActiveTab = propSetActiveTab !== undefined ? propSetActiveTab : setLocalActiveTab;

  const overdueCreditReports = useMemo(() => {
    return reports.filter(r => 
      r.status === 'sold' && 
      r.paymentType === 'credit' && 
      !r.isPaid && 
      r.creditDueDate && 
      r.creditDueDate < Date.now()
    );
  }, [reports]);
  ...
  return (
    <div className="w-full rtl text-right flex flex-col gap-4" id="admin_panel_root">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 flex flex-col gap-4" id="admin_tabs_header">
        ...
        <div className="w-full flex flex-col sm:flex-row sm:flex-wrap gap-1.5 p-1.5 bg-slate-50 border border-slate-100 rounded-2xl">
          {tabs.map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              <button
                ...
                className={`px-4 py-2.5 text-xs font-black rounded-xl transition-all flex items-center justify-center sm:justify-start gap-2.5 cursor-pointer border flex-1 min-w-[130px] ${
                  isActive
      ...
      <div className="w-full" id="admin_tab_contents">
        {activeTab === 'dashboard' && (
          <div className="flex flex-col gap-5 animate-fadeIn" id="dashboard_tab_root">
            ...
            {/* Overdue Credit Alerts Center */}
            {overdueCreditReports.length > 0 && (
              <div className="bg-red-50 border border-red-200 rounded-2xl p-4 flex flex-col gap-3 animate-fadeIn shadow-sm" id="overdue_credit_alerts">
                <div className="flex items-center gap-2 text-red-700">
                  <div className="bg-red-500 text-white rounded-lg p-1 animate-pulse">
                    <AlertTriangle className="w-5 h-5" />
                  </div>
                  <div className="flex flex-col text-right">
                    <span className="text-xs font-black">آلارم سررسید پرداخت اعتباری ({toPersianDigits(overdueCreditReports.length)} کافه)</span>
                    <span className="text-[10px] text-red-500 font-bold mt-0.5">موعد تسویه حساب فاکتورهای زیر به پایان رسیده است:</span>
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {overdueCreditReports.map(report => {
                    const daysOverdue = Math.floor((Date.now() - (report.creditDueDate || 0)) / (24 * 60 * 60 * 1000));
                    return (
                      <div key={report.id} className="bg-white rounded-xl border border-red-100 p-3 flex flex-col justify-between gap-2.5">
                        <div className="flex flex-col gap-1 text-right">
                          <span className="text-xs font-black text-slate-800">{report.cafeName}</span>
                          <span className="text-[10px] text-slate-400 font-bold">ثبت شده توسط راننده</span>
                          <div className="flex items-center justify-between text-[11px] font-bold text-slate-600 mt-1 border-t border-dashed border-slate-100 pt-1">
                            <span>مبلغ بدهی:</span>
                            <span className="text-red-600 font-black">{toPersianDigits(formatPrice(report.totalPrice))} تومان</span>
                          </div>
                          <div className="flex items-center justify-between text-[10px] font-bold text-slate-500">
                            <span>موعد پرداخت:</span>
                            <span className="font-sans text-red-500 font-black">
                              {report.creditDueDate ? getPersianDateString(new Date(report.creditDueDate).toISOString().split('T')[0]) : ''}
                            </span>
                          </div>
                          <div className="flex items-center justify-between text-[10px] font-bold text-slate-500">
                            <span>تاخیر پرداخت:</span>
                            <span className="text-red-700 font-black">{toPersianDigits(daysOverdue > 0 ? daysOverdue : 1)} روز تاخیر</span>
                          </div>
                        </div>
                        ...
                        {onUpdateReport && (
                          <button
                            type="button"
                            onClick={async () => {
                              if (confirm('آیا از تسویه حساب کامل این فاکتور اعتباری اطمینان دارید؟')) {
                                setLoading(true);
                                try {
                            ...
                            disabled={loading}
                            className="w-full py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-[10px] font-black flex items-center justify-center gap-1 shadow-sm transition-all cursor-pointer"
                          >
                            <Check className="w-3.5 h-3.5" />
                            <span>ثبت تسویه حساب نهایی</span>
                          </button>
                        )}
                      </div>
                    );
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between font-medium">
                          ...
                          <span className="font-bold text-slate-700">
                            {selectedCafe.lastVisitDate ? toPersianDigits(getPersianDateString(selectedCafe.lastVisitDate)) : 'هنوز بازدیدی انجام نشده'}
                          </span>
                        </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-500 mt-0.5 font-medium">
                        ردیابی خودکار و زنده روی نقشه دزفول
                      </p>
                    </div>
                  </div>
                    ...
                    return (
                      <div key={req.id} className="bg-white border border-red-100 rounded-xl p-3 shadow-sm space-y-2.5 text-right text-xs">
                        ...
                        <div className="text-slate-500 space-y-1 font-medium text-[11px]">
                          <p>ثبت‌کننده: <span className="font-bold text-slate-700">راننده پخش</span></p>
                          {rep && (
                            <p>
                              گزارش ثبت شده: {' '}
                              <span className="font-bold text-slate-700">
